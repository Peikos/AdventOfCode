{-# LANGUAGE OverloadedStrings #-}

module Second  where

import Control.Comonad.Store (peek)
import Data.Text (splitOn)
import Relude.Extra.Map (lookup)

import Computer

type Result = Maybe Int

input :: IO ComputerState
input = toCS . map (readMaybe . toString) . splitOn ","
    <$> readFileText "input2"

alterMem :: Value -> Value -> ComputerState -> ComputerState
alterMem a b = memOp (write 1 (Just a) . write 2 (Just b))

nounVerbMap :: ComputerState -> Map Result (Value, Value)
nounVerbMap p = fromList [ (result $ eval [] $ alterMem noun verb p, (noun, verb))
                         | noun <- [0..99]
                         , verb <- [0..99]
                         ]

result :: (ComputerState, [Maybe Value]) -> Result
result = peek 0 . cMemory . fst

test0, test1, test2, test3, test4 :: ComputerState
test0 = loadProgram [1,9,10,3,2,3,11,0,99,30,40,50]
test1 = loadProgram [1,0,0,0,99]
test2 = loadProgram [2,3,0,3,99]
test3 = loadProgram [2,4,4,5,99,0]
test4 = loadProgram [1,1,1,4,99,5,6,0,99]

firstAnswer, secondAnswer :: IO Result
firstAnswer = result . eval [] . alterMem 12 2 <$> input
secondAnswer = fmap (\(n,v) -> 100 * n + v) . lookup (Just 19690720)
             . nounVerbMap <$> input
